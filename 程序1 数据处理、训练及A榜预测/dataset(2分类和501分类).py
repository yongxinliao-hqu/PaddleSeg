    ...
    def __init__(self,
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                ############## Modified Here ###################
                if len(items) != 3: # 将原来的2改为3，多了index数据
                    ...
                    image_path = items[0]
                    label_path = items[1]
                    classification = int(items[2])
                    if classification < 0:
                        print("############## classification < 0 ###################")
                        print("classification= " + str(classification))
                        print("############## classification < 0 ###################")
                self.file_list.append([image_path, label_path,classification])
                 ############## Modified Here ###################
    def __getitem__(self, idx):
        image_path, label_path, classification = self.file_list[idx]
        if self.mode == 'test':
            ...
            label = label[:, :, 0] #获取三维数组中的第一维所有数据，作为标签
            ...
            # if max(np.unique(label)) >1:
            #     label = label/255.
            # label = label * (classification + 1)
            # label = label.astype("int64")
            ############## Modified Here ###################
            label = label[np.newaxis, :, :] #增加一个维度
            return im, label
        else:
